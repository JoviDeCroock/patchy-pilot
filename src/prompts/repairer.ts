...
export function repairPrompt(spec: string, review: ReviewResult): string {
  const issues = [
    ...review.critical_issues.map(
      (i) => `[CRITICAL] ${i.description}${i.suggestion ? ` — Suggestion: ${i.suggestion}` : ""}`,
    ),
    ...review.likely_bugs.map(
      (i) => `[BUG] ${i.description}${i.suggestion ? ` — Suggestion: ${i.suggestion}` : ""}`,
    ),
    ...review.spec_mismatches.map(
      (i) => `[SPEC] ${i.description}${i.suggestion ? ` — Suggestion: ${i.suggestion}` : ""}`,
    ),
    ...review.missing_tests.map(
      (i) => `[TEST] ${i.description}${i.suggestion ? ` — Suggestion: ${i.suggestion}` : ""}`,
    ),
  ];
